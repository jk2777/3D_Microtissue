...
# Additional tensor function for post-processing
...
def PK1(u):
    ...
    C = F.T*F
    J  = variable(det(F))
    C_bar = variable(J**(-2/3)*C)
    Ic = tr(C)
    Ic_bar = J**(-2/3)*Ic
    Psi_vol = K/2*(J-1)**2
    Psi_iso = G/2*(Ic_bar-3)
    p = diff(Psi_vol,J)
    S_bar = 2*diff(Psi_iso,C_bar)
    PK1 = J*p*inv(F.T) + J**(-2/3)*F*dev(S_bar) + gamma*beta_bulk*J*inv(F.T)
    return PK1
...
for t in gamma_list:
    ...
    PK1tensor = project(PK1(u), TT)
    PK1tensor.rename("PK1", "PK1tensor")
    Jscalar = project(J,KK)
    Jscalar.rename("J", "Jscalar")
    ...
    file_results.write(PK1tensor,t)
    file_results.write(Jscalar,t)
